namespace Main.Common
{
  ...
  public class EventAggregator : IEventAggregator
  {
    ...
    /// <inheritdoc />
    public bool TryRegisterObservable(object eventSource, string eventName)
      => TryRegisterObservable(eventSource, new[] { eventName });
    ...
    public bool TryRegisterObserver(string eventName, Type eventSourceType, Delegate eventHandler)
    {
      ...
      {
        throw new ArgumentException($"'{nameof(eventName)}' cannot be null or empty.", nameof(eventName));
      }
    ...
    {
      MethodInfo? delegateInvokeMethodInfo = eventHandlerType.GetMethod("Invoke");
      ParameterInfo[] parameterInfoList = delegateInvokeMethodInfo.GetParameters();
      if (parameterInfoList.Length == 2)
      {
        if (parameterInfoList[0].ParameterType.IsAssignableTo(typeof(object))
          && parameterInfoList[1].ParameterType.IsAssignableTo(typeof(EventArgs)))
        {
    ...
    //    {
    //      if (parameterInfoList[0].ParameterType.IsAssignableTo(typeof(object))
    //        && parameterInfoList[1].ParameterType.IsAssignableTo(typeof(TEventArgs)))
    //      {
